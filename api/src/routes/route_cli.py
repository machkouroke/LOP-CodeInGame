...
async def lop_submit(websocket: WebSocket,
    ...
    file_name = f"archive_{user.id}_{exercise_id}.zip" if kind == "solve" else f"archive_{exercise_id}.zip"
...
async def lop_download(exercise_id: str, bucket=Depends(get_bucket)):
    file_name = f"archive_{exercise_id}.zip"
    dest_path = "LOPInGame/template/"
